...
function jd_file_callback($matches){
  ...
  // Tester si jDownloads >= 1.3 installé.
...
function jd_file_latest_updated($matches){
   ...
   $filesql ="SELECT file_id FROM #__jdownloads_files WHERE {xxxx}cat_id IN (".substr($jDFPcatids,0,-1).") AND (update_active = 1) AND (modified_date >= '.$until.') ORDER BY {dado} DESC LIMIT ".$database->getEscaped($matches[2]).";";
...
function jd_file_latest_hottest($matches){
   ...
   $filesql = "SELECT file_id FROM #__jdownloads_files WHERE {xxxx}cat_id IN (".substr($jDFPcatids,0,-1).") ORDER BY {dado} DESC LIMIT ".$database->getEscaped($matches[2]).";";
   if ($matches[1] == 'latest'){
...
function jd_file_createdownload($matches){
  ...
  $fSQLquery = "SELECT * FROM #__jdownloads_files WHERE file_id = ".$database->getEscaped($matches[2]);
  $database->setQuery($fSQLquery);
  $frows = $database->loadObjectList();
  if (!$frows){
    $fSQLquery = "SELECT * FROM #__jdownloads_files WHERE file_title = '".$database->getEscaped($matches[2])."'";
    $database->setQuery($fSQLquery);
    $frows = $database->loadObjectList();
  }
  ...
  // Quitter si le fichier n'est pas publié et que l'affichage des fichier non-publiés est désactivé
  if ($jDFPshowunpublished == 0){
  ...
  // Chercher la catégorie pour voir si visiteur a des droits sur le fichier
  ...
  // Calculer si le visiteur peut télécharger
...
function jd_file_fill_downloadok($p_Template, $p_Frow, $p_Symbol_Off, $p_DownloadType, $p_dallowed){
  ...
  //Chercher catégorie du fichier pour calculer le répertoire
  ...
  // tabs or sliders when the placeholders are used
    if ((int)$jlistConfigM['use.tabs.type'] > 0){
       ...
       $l_Template = str_replace('{tabs begin}', $jd_pane->startPane('jdpane'), $l_Template);
       $l_Template = str_replace('{tab description}', $jd_pane->startPanel(JText::_('COM_JDOWNLOADS_FE_TAB_DESCRIPTION_TITLE'), 'panel1'), $l_Template);
       $l_Template = str_replace('{tab description end}', $jd_pane->endPanel(JText::_('COM_JDOWNLOADS_FE_TAB_DESCRIPTION_TITLE'), 'panel1'), $l_Template);
       $l_Template = str_replace('{tab pics}', $jd_pane->startPanel(JText::_('COM_JDOWNLOADS_FE_TAB_PICS_TITLE'), 'panel2'), $l_Template);
       $l_Template = str_replace('{tab pics end}', $jd_pane->endPanel(JText::_('COM_JDOWNLOADS_FE_TAB_PICS_TITLE'), 'panel2'), $l_Template);
       $l_Template = str_replace('{tab mp3}', $jd_pane->startPanel(JText::_('COM_JDOWNLOADS_FE_TAB_AUDIO_TITLE'), 'panel3'), $l_Template);
       $l_Template = str_replace('{tab mp3 end}', $jd_pane->endPanel(JText::_('COM_JDOWNLOADS_FE_TAB_AUDIO_TITLE'), 'panel3'), $l_Template);
       $l_Template = str_replace('{tab data}', $jd_pane->startPanel(JText::_('COM_JDOWNLOADS_FE_TAB_DATA_TITLE'), 'panel4'), $l_Template);
       $l_Template = str_replace('{tab data end}', $jd_pane->endPanel(JText::_('COM_JDOWNLOADS_FE_TAB_DATA_TITLE'), 'panel4'), $l_Template);
       $l_Template = str_replace('{tab download}', $jd_pane->startPanel(JText::_('COM_JDOWNLOADS_FE_TAB_DOWNLOAD_TITLE'), 'panel5'), $l_Template);
       $l_Template = str_replace('{tab download end}', $jd_pane->endPanel(JText::_('COM_JDOWNLOADS_FE_TAB_DOWNLOAD_TITLE'), 'panel5'), $l_Template);
       $l_Template = str_replace('{tab custom1}', $jd_pane->startPanel($jlistConfigM['additional.tab.title.1'], 'panel6'), $l_Template);
       $l_Template = str_replace('{tab custom1 end}', $jd_pane->endPanel($jlistConfigM['additional.tab.title.1'], 'panel6'), $l_Template);      
       $l_Template = str_replace('{tab custom2}', $jd_pane->startPanel($jlistConfigM['additional.tab.title.2'], 'panel7'), $l_Template);
       $l_Template = str_replace('{tab custom2 end}', $jd_pane->endPanel($jlistConfigM['additional.tab.title.2'], 'panel7'), $l_Template);
       $l_Template = str_replace('{tab custom3}', $jd_pane->startPanel($jlistConfigM['additional.tab.title.3'], 'panel8'), $l_Template);
       $l_Template = str_replace('{tab custom3 end}', $jd_pane->endPanel($jlistConfigM['additional.tab.title.3'], 'panel8'), $l_Template);
       $l_Template = str_replace('{tabs end}', $jd_pane->endPane('jdpane'), $l_Template);      
    } else {
