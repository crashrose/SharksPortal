    ...
    */
    simpleValidate : function(el) {
        if(el.get('tag') == 'select'){
            if(!el.multiple) {
                if(el.selectedIndex == -1 || el.options[el.selectedIndex].value == '-1') {
...
/* PREVEST START */
				if ($chk($(obj)) && $chk($(obj.getProperty('id')))) {
					...
					if ($chk($(customErrorEl))) {
						var customErrorText = $(customErrorEl).get('text');
						if (customErrorText != "") {
							errors = [];
							errors.push(new Element('p').set('html', customErrorText));
						}
					}
				}
/* PREVEST END */	
				...
				if ($chk($(obj)) && $chk($(obj.getProperty('id')))) {
					...
					if ($chk($(customErrorEl))) { 
						var customErrorText = $(customErrorEl).get('text');	
						if (customErrorText != "") {new Element('p').set('html',customErrorText).inject(obj.element);}} else {
