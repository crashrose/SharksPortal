...
function categoriesSave($option,$apply=0){
    ...
    $row->cat_title   = trim(stripslashes($row->cat_title));
...
function saveConfig($option,$apply=0){
    ...
    // make sure, that we have not a line feed in this text for javascript - otherwise we have problems
    $jlistConfig['countdown.text'] = trim( preg_replace(array('/\r/', '/\n/'), '', $jlistConfig['countdown.text']));   
