...
class attendanceHelper {
	public static function yes_no_box($i,$id,$status, $disabled) {
		...
		$lines = array (
					'<input  '.$disabled.' class="att_yes_radio" name="rsvp_status['.$i.']"' .$yes_checked . ' type="radio" id="status_' . $id . '_Y" value="1" onclick="$(\'rsvp_details_'.$id.'\' ).hide();$(\'rsvp_expl_'.$id.'\' ).removeClass(\'required\');$(\'rsvp_reason_'.$id.'\' ).removeClass(\'required\');">Yes',
	...
	public static function RSVP_reason($id, $rsvp_reason, $disabled, $required) {
		...
		$lines = array (
				'<select '.$disabled.' name="rsvp_reason[]" class="att_reason inputbox '.$required.'" id="rsvp_reason_' . $id .'">',
	...
	public static function RSVP_details($id, $rsvp_details, $disabled, $required) {
		// Create the batch selector to change the client on a selection list.
		$lines = '<textarea  class="att_details '.$required.'" '.$disabled.' rows="3" cols="18" name="rsvp_details[]" id="rsvp_expl_' . $id .'" />'.$rsvp_details.'</textarea>';
