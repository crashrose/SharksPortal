...
abstract class JHtmlRSVP
{
	...
	{
		return '</div></div><div class="panel" id="' . $id . '" style="display:none;"><h3 class="pane-toggler title" id="' . $id . '"><a href="javascript:void(0);"><span>' . $text
	...
	protected static function _loadBehavior($group, $params = array())
	{
		...
		if (!array_key_exists($group, $loaded))
		{
			...
			//If startOffset is set and startTransition is set and True, then use the startOffset param as display car
			$display = (isset($params['startOffset']) && isset($params['startTransition']) && $params['startTransition'])
				? (int) $params['startOffset'] : null;
			
			//If startOffset is set and startTransition is not set to True, then use the startOffset param as SHOW var
