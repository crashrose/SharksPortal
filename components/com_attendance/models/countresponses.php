...
class attendanceModelCountResponses extends JModelList {
	...
	protected function getListQuery() {
		...
		$totalquery->order($db->escape($this->getState('list.ordering', 'group_name')).' '.
	...
	protected function populateState($ordering = null, $direction = null) {
		...
		if ($filtered==1){
		$state = $filter_type == 'user_id'?'event_date':'user_id';	
		}
			else{
...
$this->setState ( 'list.groupby', $state );
		...
		parent::populateState ('group_name', 'asc' );
	}
...
// 		$filter_info = $group_type_filter!=''?', filtered_val ':', 0 as filtered_val';
		...
		switch ($group_by) {
			case 'user_id' :
				$query->select('user_name as group_name
						, user_id as group_val
						, \'' . $group_by . '\' as group_type 
						, \'User Name\' as group_type_eng' );
				$query->group('user_name');
				break;
				case 'event_date' :
					$query->select('event_date as group_name
						, event_date as group_val 
						, \'' . $group_by . '\' as group_type
	...
	public function populateSubQuery(&$subquery, $cmpl, $event_month, $regex, $group_val_filter, $group_type_filter) {
		$subquery->select ( 'distinct event.event_name, event.id as event_id
                ...
                , 1 as event_count, event.event_date as event_date, loc.loc_name as event_loc, type.event_type_name as event_type
		...
		switch ($cmpl) {
			...
			default: $subquery->where( 'responses.id is null OR (rsvp_active=1 and rsvp_status=-1)');break;
		}
...
// 		echo str_replace('_id', '.id', $group_type_filter)) . '='. $group_val_filter;
		...
		switch ($group_type_filter){
			case 'event_date':
				$subquery->select( $group_type_filter . ' as filtered_val');
				$subquery->where( ' event_date = \''.$group_val_filter.'\'');
				break;	
			case 'user_id':
				$subquery->select( 'user.name as filtered_val');
				$subquery->where( ' user.id =' .$group_val_filter);
				break;
			default:
