...
/**
 * Import Controller Importing a file follows this process: 1. controllers/importfile.php -> importFile 2. models/importfile.php -> prepareImport (sets session values) 3. views/importfile/view.html.php -> display 4. views/importfile/tmpl/default.php JS calls import 5. controllers/importfile.json.php -> doImport 6. models/importfile.php -> getDoImport (sets session values) 7. views/importfile/view.json.php -> return result
 * @package CSVI
 */
				...
				foreach ( $event_active as $key => $val ) {
					...
					if (! $no_change && ! $default_record) {
						$submitted_responses [] = array (
								'rsvp_details' => $rsvp_status [$key] == '1' ? '' : $rsvp_details [$key],
								'rsvp_reason' => $rsvp_status [$key] == '1' ? '' : $rsvp_reason [$key],
								...
								'rsvp_active' => 1,
								'rsvp_updated_by' => JFactory::getUser ()->id,
								'id' => ''
						);
					}
				}
